...
final class VesselPhysicsScene: SKScene {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        guard !renderer.blocksHit(at:point) else { return }
        ...
        }), let node = nodesByID[id], let body = node.physicsBody else { return }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let state = drag, let touch = touches.first(where:{ $0 === state.touch }) else { return }
