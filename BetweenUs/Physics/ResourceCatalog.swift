...
// 唯一的产品配置入口：增加皮肤只登记映射，增加物件只登记新配方。
// 几何、尺寸、质量、重力、投放和拖拽参数沿用已验证的标定，不在接入时重调手感。
enum ResourceCatalog {
    ...
    static func recipe(for kind: ContainerKind) -> VesselRecipe {
        switch kind {
        case .star: return starJar
        case .capsule: return capsuleBox
        case .paper: return paperBin
        }
    }

    // 内容标识决定皮肤，同一件内容每次进入都拿到同一张图；不使用进程内随机哈希。
    // 抽屉、物理舞台和卡片图标都从这里取皮肤，界面里不会出现第二套贴图映射。
    static func tokenSkin(for kind: ContainerKind, id: UUID? = nil) -> SkinProfile {
        let skins = recipe(for: kind).skins
        guard let id else { return skins[0] }
        let bytes = withUnsafeBytes(of: id.uuid) { Array($0) }
        let digest = bytes.reduce(0) { ($0 &* 31 &+ Int($1)) & 0x7fff_ffff }
        return skins[digest % skins.count]
    }

    static func tokenSkinID(for kind: ContainerKind, id: UUID) -> String {
        tokenSkin(for: kind, id: id).id
    }
    ...
    // 五系列六情绪的挂件共用同一个星星轮廓，只换皮肤不复制几何。
    private static let starSkins: [SkinProfile] = StarCharm.all.map { charm in
        SkinProfile(id: charm.imageName, entityID: "rounded-star", asset: charm.imageName,
                    sourceSize: .init(width: 256, height: 256),
                    contentRect: .init(x: 0, y: 0, width: 256, height: 256))
    }

    private static let paperSkins: [SkinProfile] = (1...10).map { index in
        let name = String(format: "PaperBall_%02d", index)
        return SkinProfile(id: name, entityID: "paper-ball", asset: name,
                           sourceSize: .init(width: 256, height: 256),
    ...
    static let capsuleBox = VesselRecipe(
        ...
        skins:[SkinProfile(id:"Capsule_Closed",entityID:"capsule",asset:"Capsule_Closed",
                           sourceSize:.init(width:256,height:256),
